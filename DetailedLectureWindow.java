...
public class DetailedLectureWindow extends JFrame {
    ...
    private Map<String, List<String>> parsePeriodFormat(String lectureTime) {
        ...
        for (String part : parts) {
            ...
            System.out.println("처리 중인 부분: '" + part + "'");
            
            // 요일인지 확인
            if (part.matches("[월화수목금토일]")) {
                currentDay = part;
                // 같은 요일이 반복되는 경우를 위해 기존 리스트에 추가
                if (!dayToPeriods.containsKey(currentDay)) {
                    dayToPeriods.put(currentDay, new ArrayList<>());
                }
                System.out.println("요일 발견: " + currentDay);
            } else if (currentDay != null && part.matches("\\d+[AB]?")) {
                // 교시 형태 (6A, 6B, 7A, 7B 등)
                dayToPeriods.get(currentDay).add(part);
                System.out.println("교시 추가: " + currentDay + " -> " + part);
            }
        }
